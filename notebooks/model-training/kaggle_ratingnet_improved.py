...
# 
# Cải tiến từ paper **arXiv:2409.11506** (*Chess Rating Estimation from Moves and Clock Times*).
# 
# **Điểm cải tiến so với paper gốc:**
# - Thêm **Centipawn Loss (CPL)** sequence từ Stockfish
# - Thêm **Blunder flag** (CPL > 200)
# - Concat `[board_vec, clock, cpl, blunder]` trước LSTM
# 
# **Kiến trúc giữ nguyên paper:**
# - 4 Conv2D + BatchNorm + LeakyReLU + AvgPool
# - Bi-LSTM 3 layers (hidden=64)
# - FC → `[WhiteELO, BlackELO]`
...
CONFIG = {
    # --- Đường dẫn dữ liệu (SỬA CHO KAGGLE) ---
    "data_path": "/kaggle/input/chess-data/sample_30k_dl.parquet",

    # --- Feature flags (bật/tắt để ablation) ---
    "use_clock": True,       # Clock time (paper gốc)
    "use_cpl": True,         # CPL sequence (cải tiến)
    "use_blunder": True,     # Blunder flag (cải tiến)
...
# ## 1. Board Encoding
# 
# Mỗi vị trí bàn cờ → tensor `[12, 8, 8]` (12 bitboard planes: 6 quân × 2 màu).
...
def replay_game_to_boards(moves_san: str, max_moves: int = 150) -> list:
    ...
    board = chess.Board()
    boards = []
    # Lọc bỏ số thứ tự nước đi và kết quả
...
# ## 2. Dataset & Collate
# 
# Đọc parquet → replay ván cờ → trả về `{positions, clocks, cpls, blunders, targets, length}`.
# 
# **Cột parquet cần có:**
# - `Moves` (SAN string) — bắt buộc
# - `WhiteElo`, `BlackElo` — bắt buộc
# - `TimeControl` (e.g. "180+0") — optional, dùng phân loại thể thức
# - `ClockSeq` (JSON array giây) — optional
# - `CplSeq` (JSON array CPL) — optional
...
class ChessGamesDataset(Dataset):
    ...
    def __init__(self, df, max_moves=150, config=None):
        """
        Args:
            df: pandas DataFrame chứa ván cờ.
            max_moves: Số nước tối đa (truncation).
            config: Dict cấu hình (normalization params, feature flags).
        """
        self.df = df.reset_index(drop=True)
        self.max_moves = max_moves
        self.config = config or CONFIG

        # Kiểm tra cột nào có trong data
        self.has_clock = "ClockSeq" in self.df.columns
        self.has_cpl = "CplSeq" in self.df.columns
        self.has_time_control = "TimeControl" in self.df.columns

        if not self.has_clock:
            print("  ⚠ Không tìm thấy cột ClockSeq → clock = 0")
        if not self.has_cpl:
            print("  ⚠ Không tìm thấy cột CplSeq → cpl/blunder = 0")
    ...
    def __getitem__(self, idx):
        ...
        if self.has_clock and self.config["use_clock"]:
            try:
                raw = json.loads(str(row["ClockSeq"])) if pd.notna(row["ClockSeq"]) else []
                for i, c in enumerate(raw[:length]):
                    if c is not None:
                        clocks[i] = (c - self.config["clocks_mean"]) / self.config["clocks_std"]
        ...
        if self.has_cpl:
            try:
                raw = json.loads(str(row["CplSeq"])) if pd.notna(row["CplSeq"]) else []
                for i, c in enumerate(raw[:length]):
                    if c is not None:
                        if self.config["use_cpl"]:
                            cpls[i] = (c - self.config["cpl_mean"]) / self.config["cpl_std"]
                        if self.config["use_blunder"]:
                            blunders[i] = 1.0 if c > self.config["blunder_threshold"] else 0.0
            except (json.JSONDecodeError, TypeError):
                pass
...
# %% [markdown]
# ## 3. Model Architecture — RatingNet (Cải tiến)
#
# Giữ nguyên CNN 4 tầng + BiLSTM từ paper.
# Thêm CPL + Blunder vào concat layer trước LSTM.
#
# ```
# [Board 12×8×8] → CNN (4 Conv+BN+Pool) → [256-dim vector]
#                                              ↓
# [clock, cpl, blunder] ──────────────→ Concat → [259-dim]
#                                                   ↓
#                                              Bi-LSTM (3 layers)
#                                                   ↓
#                                         FC → [WhiteELO, BlackELO]
...
class ChessEloPredictor(nn.Module):
    ...
    Kiến trúc CNN giữ nguyên paper (4 Conv2D + BN + AvgPool).
    LSTM input = conv_filters*8 + n_extra_features.
    Paper gốc: n_extra = 1 (clock).
    Cải tiến:   n_extra = 3 (clock + cpl + blunder).
    """
    ...
    def __init__(self, config=None):
        ...
        # ── LSTM ──
        # CNN output: cf*8 (sau 3 lần pool 8→4→2→1, conv4 trên 1×1)
    ...
    def forward(self, positions, clocks, cpls, blunders, lengths):
        """
        Args:
            positions: [B, T, 12, 8, 8]
            clocks:    [B, T]
            cpls:      [B, T]
            blunders:  [B, T]
            lengths:   [B] (CPU tensor)
        Returns:
            all_outputs:  [B, T, 2] — prediction mỗi time-step
            last_output:  [B, 2]    — prediction nước cuối
        """
        ...
        x = self.pool(x)                              # 2→1
        x = F.leaky_relu(self.bn4(self.conv4(x)))     # Không pool (giữ 1×1)
        x = self.dropout(x)
        ...
        parts = [x, clocks.unsqueeze(2)]              # clock: [B, T, 1]
        if self.use_cpl:
            parts.append(cpls.unsqueeze(2))            # cpl: [B, T, 1]
        if self.use_blunder:
            parts.append(blunders.unsqueeze(2))        # blunder: [B, T, 1]
        lstm_input = torch.cat(parts, dim=2)           # [B, T, cf*8 + extra]
        ...
        # FC trên TẤT CẢ time-steps (giống paper)
...
def train_one_epoch(model, loader, optimizer, criterion, device, config):
    ...
    for batch in loader:
        ...
        # Rescale về ELO gốc trước khi tính MAE (giống paper)
...
def test_with_breakdown(model, loader, criterion, device, config):
    """Test + MAE breakdown theo thể thức (giống paper)."""
    model.eval()
    total_loss = 0.0
    rm, rs = config["ratings_mean"], config["ratings_std"]
    loss_by_tc = {}
...
def main(config=None):
    ...
    print(f"\n{'═' * 60}")
    print(f"  ♟️  RatingNet Training (Paper + CPL/Blunder)")
    print(f"{'─' * 60}")
    ...
    criterion = nn.L1Loss()  # MAE — giống paper
...
# %% [markdown]
# ## 6. Ablation: So sánh có/không CPL
#
# Bỏ comment cell dưới để chạy ablation (tắt CPL, chỉ giữ Clock như paper gốc).
